...
Используйте скрипт `start_process.py`, который автоматически установит браузеры Playwright и запустит сервер:
...
Этот скрипт:
1. Установит системные зависимости (для Linux)
2. Установит браузеры Playwright (Chromium)
3. Запустит API сервер

#### Ручной запуск:

Запустите API сервер напрямую:
```bash
...
**Важно:** Перед первым запуском убедитесь, что установлены браузеры Playwright:
```bash
playwright install chromium
```
...
├── start_process.py    # Скрипт для автоматической установки и запуска
